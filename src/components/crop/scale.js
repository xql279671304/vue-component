...
ImagesZoom.prototype = {
  ...
  init: function (param) {
    var self = this,
      params = param || {},
      zoomImg = params.elem,
      cropBtn = document.querySelector('.crop-btn .sure-crop')
  ...
  reset: function () {
    const self = this
    let hideTime = '.2s',
      t = self.crop.t - self.element.offsetTop
    if (self.distY > t) {
      self.newY = t
...
/* eslint-env es6 */
  ...
  cropSize: function (param) {
    ...
    if (params.ratio) {
      let size = params.ratio.split(':'),
        aspectRadio = size[0] / size[1],
        screenRadio = clientWidth / clientHeight
  ...
  drawImageFn: function () {
    ...
    let imgL = (self.imgBaseWidth - crop.w) / 2,
      imgT = (self.imgBaseHeight - crop.h) / 2
    imgL = imgL - self.newX
    imgT = imgT - self.newY
